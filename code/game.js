...
Chicken.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Kuro.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  else if (type == "chicken" && this.status == null) {
    this.status = "lost";
    this.finishDelay = .5; }
    // If player touches Kuro, player wins
  else if (type == "kuro" && this.status == null)  {
      this.status = "won";
      this.finishDelay = 1; }
